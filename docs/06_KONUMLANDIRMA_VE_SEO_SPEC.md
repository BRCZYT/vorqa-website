# Vorqa — Konumlandırma, İçerik ve SEO/AEO Uygulama Spec'i
...
> **📝 EDİTÖR NOTU (güncelleme):** Bu belge orijinal olarak "ZYT" markası için yazılmıştı. Belge yazıldıktan sonra proje **"Vorqa Global"** markasına geçti (site, domain, llms.txt, sosyal handle'lar hâlâ canlı hepsi Vorqa). Aşağıdaki metinde marka adı/domain referansları Vorqa'ya güncellendi. **Ama §5'teki "aile baş harfleri" köken hikâyesi ZYT'ye özgüydü** ("Z-Y-T" gerçek aile baş harfleriydi) — bu çerçeve "Vorqa" için otomatik geçerli değil; Burcu'nun onayı/girdisi olmadan "Vorqa" için kişisel bir köken anlamı uydurulmadı. §5'i yeniden değerlendir.
- **Marka KORUNUYOR (orijinal karar, artık "Vorqa" için geçerli):** Rebrand yapılmayacaktı — ama fiilen Vorqa'ya geçildi. Bu belgenin geri kalanı, o zamanki "ismi koru + konumlanma/iddiaları yeniden kur" stratejisinin şimdi Vorqa'ya uygulanmış hali olarak okunmalı.
- **Ama iki eski çerçeve İPTAL:**
  1. "Grup şirketi / çatı kuruluş" konumu → İPTAL. Yerine: **teknik tedarik & proje entegrasyon partneri.**
  2. `05_MARKA_GORSEL_DIL_REHBERI.md`'deki *"Tüm tarih ve uzmanlık Vorqa'nın tekil hikayesidir"* kuralı → **DEĞİŞTİ.** (Aşağıda §6.3.)
...
> **KURAL — 05'teki "MUTLAK KURAL"ın yeni hali:** Alt şirket *isimleri* (faz 1'de) yine hiçbir yerde geçmez. AMA onların sicili / sertifikası / tamamladığı proje, Vorqa'nın *kendi* geçmişiymiş gibi sunulmaz. "İsim vermemek" ile "başkasının sicilini sahiplenmek" farklı şeylerdir. Birincisi normal, ikincisi yanıltıcı.
...
| Marka | **Vorqa korunur** (isim netleşti: eski çalışma adı "ZYT" yerine "Vorqa" fiilen benimsendi; konumlanma/metin/iddialar bu belgedeki mantıkla yeniden kuruldu) |
| Marka string'i | **"Vorqa Global"** (aşağıda §1.1) |
...
| Birincil domain | **vorqaglobal.com** (canlı) |
| Ön koşul | **Marka taraması (TÜRKPATENT/WIPO/EUIPO)** — "Vorqa" için tekrar/doğrulanmalı, domain zaten alınmış olsa da tescil süreci ayrı konu (§10) |

### 1.1 Marka string'i: "Vorqa Global"
**Her yerde tek tutarlı string kullan: "Vorqa Global".** ("Vorqa Global Industry" / "…Group" / bare "Vorqa" tek başına kullanma.) Tutarlılık AEO'nun temelidir — aynı varlık her yerde aynı adla anılmalı: site, JSON-LD, llms.txt, sosyal bio, e-posta imzası.
...
4. **Tek muhatap, çok-kalemli proje** — rakiplerin çoğu tek ürün/tek işlem. Vorqa çok-kalemli yatırımı (beton+çelik+galvaniz+enerji) tek yönetilen pakette, tek sözleşmede toplar. Daha zor, daha değerli.
...
> **⚠️ BU BÖLÜM ZYT'YE ÖZGÜYDÜ, DOĞRUDAN VORQA'YA TAŞINAMAZ:** Orijinal metin "ZYT, ailenin gerçek baş harfleridir" iddiasına dayanıyordu. "Vorqa" bir baş harf kısaltması gibi görünmüyor — bu yüzden burada "adı ailenin baş harflerini taşır" cümlesini olduğu gibi bırakmadık. **Burcu'dan gerçek bilgi gerekiyor: "Vorqa" adının kökeni/kişisel anlamı nedir?** Cevaba göre bu bölüm ya kişisel-anlam çerçevesiyle yeniden yazılır, ya da (eğer "Vorqa" saf bir uydurma/coined isimse) köken hikâyesi tamamen çıkarılıp kurucunun gerçek deneyimine (§6.2) odaklanılır.

- ❌ **DEME (hâlâ geçerli):** "30 yıllık aile firması" / "köklü aile mirası" / uzun kurumsal geçmiş iması. → Bu, şişirilmiş-geçmiş tuzağına geri döner.

Kural: **Kişisel/duygusal anlam EVET (eğer gerçekse); uzun kurumsal sicil iddiası HAYIR.**
...
| ISO/CE/EN 1090 sertifika duvarı (Vorqa'nınmış gibi) | Sahip değil / doğrulanmadı — **en ciddi hukuki risk** |
| "en büyük", "lider", "dünya…" | Kanıtsız üstünlük |
| Enerji "1–50 MW", çelik "500+ ton/yıl" (Vorqa kapasitesiymiş gibi) | Partner kapasitesi — Vorqa'nınmış gibi sunma |
...
> **Kritik ayrım:** Partner ismi vermemek = normal ve dürüst (sourcing ajansları tedarikçi listesini paylaşmaz). Sorun hiçbir zaman "isim vermek" değildi — sorun **başkasının projesini/sertifikasını/sicilini Vorqa'nın kendisiymiş gibi göstermekti.**
...
- **Tek e-posta — bu madde çözüldü.** Orijinal denetimde 4 çelişkili adres vardı (eski domain'lere ait varyantlar + genel placeholder'lar); artık tek, tutarlı `info@vorqaglobal.com` kullanılıyor (llms.txt'te doğrulandı). `purchasing@vorqaglobal.com` gibi ek adresler gerekirse aynı domain altında eklensin.
...
- **Gerçek telefon/adres — bu madde çözüldü.** Gerçek telefon (`+90 536 707 69 51`) ve açık adres (OSTİM, Yenimahalle, Ankara) llms.txt'te dolu; aynı değerlerin footer/JSON-LD/GBP'de birebir eşleştiğini doğrula.
...
## İletişim
- Adres: Uzay Çağı Caddesi, OSTİM, Yenimahalle, Ankara, Türkiye
- Telefon: +90 536 707 69 51
- E-posta: info@vorqaglobal.com
- Web: https://www.vorqaglobal.com
```
*(Not: Bu blok, canlı `/llms.txt` dosyasındaki gerçek, doldurulmuş verilerle senkronize edildi — placeholder'lar artık kalmadı.)*
...
```html
<link rel="canonical" href="https://www.vorqaglobal.com/en/galvanizing/" />
<link rel="alternate" hreflang="tr" href="https://www.vorqaglobal.com/tr/galvaniz/" />
<link rel="alternate" hreflang="en" href="https://www.vorqaglobal.com/en/galvanizing/" />
<link rel="alternate" hreflang="ar" href="https://www.vorqaglobal.com/ar/galvanizing/" />
<link rel="alternate" hreflang="x-default" href="https://www.vorqaglobal.com/en/galvanizing/" />
```
...
Sitemap: https://www.vorqaglobal.com/sitemap.xml
```
...
*`LocalBusiness` yalnızca **gerçek, doğrulanabilir açık adres** girilince kullanılır (aksi halde `Organization` ile sınırlı kal). `Organization` içindeki `name` = "Vorqa Global", `url`, `logo`, `email`, `address` (PostalAddress, gerçek), `areaServed` (MENA/Afrika), `sameAs` (sosyal profiller — açılınca).

### 8.4 `<title>` + meta description deseni
Desen: **`{Birincil anahtar kelime} — {Fayda/Coğrafya} | Vorqa Global`**. Alt-marka adıyla başlama.
| Sayfa | TR title | EN title |
|---|---|---|
| Ana sayfa | `Türk Endüstriyel Üretici Tedarik Partneri — MENA & Afrika \| Vorqa Global` | `Turkish Industrial Sourcing Partner for MENA & Africa \| Vorqa Global` |
| Galvaniz | `Sıcak Daldırma Galvaniz Tesisi Tedariki — Türkiye \| Vorqa Global` | `Hot-Dip Galvanizing Plant Supplier — Turkey to MENA \| Vorqa Global` |
| Beton | `Beton Santrali Tedarik & Kurulum — Türkiye \| Vorqa Global` | `Concrete Batching Plant Supplier — Turkey to MENA \| Vorqa Global` |
| Enerji | `Atıktan Enerji & Biyogaz Tesisi Tedariki \| Vorqa Global` | `Waste-to-Energy & Biogas Plant Sourcing — MENA \| Vorqa Global` |
| Çelik | `Çelik Konstrüksiyon & Mekanik İmalat Tedariki \| Vorqa Global` | `Steel Fabrication Supplier — Turkey \| Vorqa Global` |
| Tedarik | `Endüstriyel Tedarik & Proje Koordinasyonu — Türkiye \| Vorqa Global` | `Industrial Procurement Partner — Turkey & MENA \| Vorqa Global` |
...
3. **Çözüm alanları (capabilities)** — beton / galvaniz / enerji / çelik / tedarik. Her biri `Service` şema. ("Vorqa Beton" gibi sahte "şirket" değil; "Beton Santrali Tedarik & Kurulum Koordinasyonu" gibi hizmet.)
...
1. **Marka taraması (her şeyden önce, durum: muhtemelen yapılmadı — tekrar kontrol et).** Bir marka vekiliyle TÜRKPATENT + WIPO Global Brand DB + EUIPO'da ilgili Nice sınıflarında (**6, 7, 37, 40, 42**) "Vorqa" (kelime + logo) taranır. Ucuz, hızlı, kesin. → *Not: Orijinal belgede burada ZYT'ye özgü bilinen çakışma örnekleri (Çinli ZYT çelik/enerji, ZYT Petroleum, ZYT otomotiv) vardı — bunlar "Vorqa" için geçerli DEĞİL ve uydurulmadı. "Vorqa" için sınıf 6/7/37/40/42'de gerçek bir çakışma taraması henüz yapılmamış olabilir; bu adım hâlâ gerekli.*
2. **Domain ve marka:** `vorqaglobal.com` zaten alınmış ve canlı. First-to-file olduğu için marka tescil başvurusunu (yukarıdaki tarama temizse) vekille başlat.
3. **Tek kimlik sabitleme:** Gerçek NAP + tek e-posta tüm yüzeylere (§7) — **bu adım tamamlandı** (llms.txt'de gerçek kurucu adı, adres, telefon dolu).
4. **İçerik + SEO uygulaması:** Bu belge + `03`/`05` (görsel) ile Claude Code'da uygula: §6 içerik temizliği, §8 SEO seti, §9 yeni IA, §7 AEO/llms.txt. Dil bazlı URL yapısı (§8.1) **fiilen hayata geçirilmiş görünüyor** (tr/en/ar klasörleri mevcut) — yine de hreflang/canonical doğruluğunu tekrar test et.
...
- ❔ "Vorqa" markasının ilgili sınıflarda tescil durumu → **marka vekili taraması gerekiyor** (Adım 1) — henüz yapıldığı doğrulanmadı.
- ✅ `vorqaglobal.com` — canlı ve kullanımda (registrar doğrulaması artık gereksiz).
...
- ✅ NAP + kurucu adı placeholder'ları → gerçek değerlerle dolduruldu (llms.txt'te doğrulandı).
- ❔ "Vorqa" adının kişisel/kurumsal kökeni → §5'te işaretlendi, Burcu'nun girdisi gerekiyor.
